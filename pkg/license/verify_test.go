package license

import "testing"

func TestGetMachineID(t *testing.T) {
	machineID := GetMachineID("test")
	t.Logf("machineID: %s", machineID)
}

func TestVerifyLicense(t *testing.T) {
	licenseStr := ""
	pubKeyStr := "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDOaK7ymIYlb6Yo3PWf/aGeez2ehmVUaUjMDU9NBtfpK"

	err := StoreLicense("test", "apps", licenseStr)
	if err != nil {
		t.Error(err)
	}
	secretLicense, _ := RetrieveLicense("test", "apps")
	err = VerifyLicense("test", secretLicense, pubKeyStr)
	if err != nil {
		t.Error(err)
	}

	t.Logf("verify license success")
}

func TestReadLicense(t *testing.T) {
	licenseStr := ""
	pubKeyStr := "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDOaK7ymIYlb6Yo3PWf/aGeez2ehmVUaUjMDU9NBtfpK"

	info, err := Read(licenseStr, pubKeyStr)
	if err != nil {
		t.Error(err)
	}
	t.Logf("info: %v", info)
}
